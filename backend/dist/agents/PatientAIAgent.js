...
class PatientAIAgent {
    ...
    async processMessage(message) {
        try {
            logger_1.logger.info(`Patient AI Agent processing message: ${message.id}`);
            ...
            logger_1.logger.error('Patient AI Agent error:', error);
    ...
    Format as JSON with fields: riskLevel, riskScore, recommendedAction, specialization, urgency, explanation`;
        const analysisResult = await this.openAI.generateResponse('You are a medical triage AI. Analyze symptoms and provide structured risk assessment.', analysisPrompt, message.fromUserId);
        let analysis;
        try {
    ...
    async bookAppointment(message) {
        ...
        try {
            ...
            // Send confirmation
            await this.notificationService.sendAppointmentConfirmation(message.fromUserId, appointment);
            return {
                id: this.generateId(),
                agentType: 'patient',
                fromUserId: 'patient-ai-agent',
            ...
            logger_1.logger.error('Appointment booking error:', error);
            return this.createErrorResponse(message, 'Unable to book appointment. Please try again or contact support.');
        }
    }
    async autoBookUrgentAppointment(patientId, analysis) {
        try {
            // Find available doctor for urgent consultation
            const availableDoctor = await this.appointmentService.findAvailableUrgentDoctor(analysis.suggestedSpecialization);
            if (availableDoctor) {
            ...
            logger_1.logger.error('Auto-booking urgent appointment failed:', error);
        }
    }
    calculateWaitTime(riskLevel) {
        switch (riskLevel) {
    ...
    formatSymptomAnalysisResponse(analysis) {
...
exports.PatientAIAgent = PatientAIAgent;
//# sourceMappingURL=PatientAIAgent.js.map