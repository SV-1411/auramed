...
class HealthInsightsService {
    ...
    async generateHealthInsights(patientId) {
        try {
            ...
            logger_1.logger.info(`Generated ${insights.length} health insights for patient ${patientId}`);
            return insights;
        }
        catch (error) {
            logger_1.logger.error('Failed to generate health insights:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async analyzeAppointmentPatterns(patient) {
        const insights = [];
        ...
        const daysSinceLastAppointment = Math.floor((new Date().getTime() - new Date(lastAppointment.scheduledAt).getTime()) / (1000 * 60 * 60 * 24));
     ...
     */
    async analyzeRiskFactors(patient) {
        const insights = [];
     ...
     */
    async generatePreventiveCareInsights(patient) {
        const insights = [];
     ...
     */
    async generatePredictiveAnalysis(patientId) {
        try {
            const patient = await prisma.user.findUnique({
                where: { id: patientId },
                include: {
            ...
            logger_1.logger.error('Failed to generate predictive analysis:', error);
            throw error;
        }
    }
    /**
     ...
     */
    extractChronicConditions(medicalRecords) {
        const conditions = [];
     ...
     */
    calculateDiabetesRisk(age, medicalHistory) {
        let risk = 0;
        if (age > 45)
            risk += 0.3;
        if (age > 65)
            risk += 0.2;
        if (medicalHistory.some(h => h.toLowerCase().includes('pre-diabetes')))
            risk += 0.4;
        if (medicalHistory.some(h => h.toLowerCase().includes('obesity')))
            risk += 0.3;
     ...
     */
    calculateHypertensionRisk(age, medicalHistory) {
        let risk = 0;
        if (age > 40)
            risk += 0.2;
        if (age > 60)
            risk += 0.3;
        if (medicalHistory.some(h => h.toLowerCase().includes('high blood pressure')))
            risk += 0.5;
        return Math.min(risk, 1);
    }
    /**
     ...
     */
    calculateCardiovascularRisk(age, medicalHistory) {
        let risk = 0;
        if (age > 50)
            risk += 0.2;
        if (age > 65)
        ...
        if (medicalHistory.some(h => h.toLowerCase().includes('heart')))
            risk += 0.4;
        if (medicalHistory.some(h => h.toLowerCase().includes('cholesterol')))
            risk += 0.2;
        return Math.min(risk, 1);
    }
    /**
     ...
     */
    calculateObesityRisk(medicalHistory) {
        let risk = 0;
        if (medicalHistory.some(h => h.toLowerCase().includes('obesity')))
            risk += 0.6;
        if (medicalHistory.some(h => h.toLowerCase().includes('overweight')))
            risk += 0.3;
        return Math.min(risk, 1);
    }
    /**
     ...
     */
    generateRiskBasedRecommendations(riskFactors) {
    ...
    calculateNextCheckupDate(age, riskFactors) {
        ...
        const highRisk = Object.values(riskFactors).some((risk) => risk > 0.5);
        if (highRisk || age > 65) {
     ...
     */
    generatePreventiveMeasures(age, riskFactors) {
        const measures = [
        ...
        if (Object.values(riskFactors).some((risk) => risk > 0.3)) {
...
//# sourceMappingURL=HealthInsightsService.js.map